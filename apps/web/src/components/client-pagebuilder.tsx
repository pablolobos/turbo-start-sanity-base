...
// Client-only version of PageBuilder without useOptimistic
export default function ClientPageBuilder({
    pageBuilder: initialPageBuilder = [],
    id,
    type,
}: PageBuilderProps) {
    // Store the pageBuilder blocks in state 
    ...
    return (
        ...
        >
            {blocks.map(block => renderBlock(block, id, type))}
        </main>
    );
} 